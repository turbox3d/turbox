...
import { Domain, Vector2, Vector3, Box2, EntityObject, Box3, mutation, Action, Vec2 } from '@byted-tx3d/turbox';

import { appCommandManager } from '../../commands/index';
import { CameraDistance, Z_INDEX_ACTION, MIRROR_ACTION, RenderOrder } from '../../common/consts/scene';
...
export class ActionsDomain extends Domain {
  ...
  @mutation
  copy = () => {
    const selected = appCommandManager.defaultCommand.select.getSelectedEntities() as ProductEntity[];
    selected
  ...
  @mutation
  lock() {
    const selected = appCommandManager.defaultCommand.select.getSelectedEntities()[0] as ProductEntity | BackgroundEntity;
    if (
  ...
  replace = async <CustomBizData>(url: string, target?: EntityObject, extraInfo?: CustomBizData, isSelf = false) => {
    const selected = (target ||
      appCommandManager.defaultCommand.select.getSelectedEntities()[0]) as ProductEntity<CustomBizData>;
  ...
  group = () => {
    ...
    if (selected.length === 1 && EntityCategory.isAssembly(selected[0])) {
      ...
      assembly.children.forEach(child => {
        ...
        appCommandManager.defaultCommand.select.select([child], false);
      });
    ...
    appCommandManager.defaultCommand.select.select([assembly]);
  };
  ...
  updateMirror = (type: MIRROR_ACTION) => {
    const selected = appCommandManager.defaultCommand.select.getSelectedEntities()[0] as ProductEntity;
    if (!EntityCategory.isProduct(selected)) {
      return;
    }
  ...
  skew = async () => {
    ...
    const blob = (await SceneUtil.getScreenShot(
      0,
      0,
    ...
    appCommandManager.skewCommand.apply();
  };

  confirmSkew = async (uploadMethod: ({ data, fileName }: { data: any; fileName: string }) => Promise<string>) => {
    ...
    appCommandManager.defaultCommand.apply();
  ...
  clip = () => {
    ...
    appCommandManager.clipCommand.apply();
  };

  confirmClip = async (uploadMethod: ({ data, fileName }: { data: any; fileName: string }) => Promise<string>) => {
  ...
  clearScene = () => {
    ...
    ldeStore.scene.$update({
      cameraPosition: new Vector3(0, 0, CameraDistance.CAMERA),
  ...
  save = <CustomBizData>({ spaceId = '', cartId = '', id = '', thumbnailUrl = '' }) =>
    ldeStore.document.dumpDocument<CustomBizData>({
      spaceId,
      cartId,
      id,
      thumbnailUrl,
  ...
  screenShot = async (download = false, fileType = 'image/jpeg', quality = 1, zoom = 1) => {
    const selected = appCommandManager.defaultCommand.select.getSelectedEntities().slice();
    const rootView3d = SceneUtil.getRootView() as THREE.Group;
    ...
    const blob = (await SceneUtil.getScreenShot(
      Math.floor(sx * resolution),
      Math.floor(sy * resolution),
      Math.floor(width * resolution),
      Math.floor(height * resolution),
      fileType,
      quality,
    ...
    mutation(
      ...
      () => {
        ...
        appCommandManager.defaultCommand.select.select(selected);
      },
      { immediately: true }
    )();
    return blob;
