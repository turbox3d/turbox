...
import {
  ...
  CoordinateType,
  GesturesExtra
} from '@byted-tx3d/turbox';

// import { MathAlg, Line2d } from '@byted-tx3d/homestyler-math';
import { ScalePointSymbol, AdjustPointSymbol } from '../../../common/consts/scene';
...
export class ScaleCommand extends Command {
  ...
  protected onDragStart(viewEntity: ViewEntity, event: SceneEvent, tools: SceneTool) {
    ...
    if (
      (viewEntity.type === ScalePointSymbol || viewEntity.type === AdjustPointSymbol) &&
      ...
      if (viewEntity.type === AdjustPointSymbol) {
        ...
        const localPoint = new Vector3(mp.x, mp.y, mp.z).applyMatrix4(
          this.currentProduct.getConcatenatedMatrix().inverted()
        );
  ...
  protected onPinchStart(viewEntity: ViewEntity, event: SceneEvent, tools: SceneTool) {
    scaleAndRotateAction.statusFlag.scale = true;
    const selected = appCommandManager.defaultCommand.select.getSelectedEntities()[0];
    ...
    if (
      ...
      const eventCache = (event.extra as GesturesExtra).eventCache || [];
  ...
  protected onPinch(viewEntity: ViewEntity, event: SceneEvent, tools: SceneTool) {
    if (!this.currentProduct) {
      return;
    }
    if (
      this.currentProduct instanceof THREE.Group &&
      this.pinchScaleOffset &&
      this.pinchScaleInitPosition &&
      this.pinchScaleInitScale
    ) {
      // 缩放画布
      let ratio = (event.extra as GesturesExtra).scale || 1;
      ...
      scaleAndRotateAction.action.execute(
        () => {
          const product = this.currentProduct as ProductEntity;
          const scale = (event.extra as GesturesExtra).scale || 1;
          const deltaScale = {
            x: (this.initSize!.x * scale) / product.size.x,
            y: (this.initSize!.y * scale) / product.size.y,
          };
  ...
  protected onPinchEnd(viewEntity: ViewEntity, event: SceneEvent) {
    scaleAndRotateAction.statusFlag.scale = false;
  ...
  protected onWheel(viewEntity: ViewEntity, event: SceneEvent, tools: SceneTool) {
    if (ldeStore.scene.isSkewMode || ldeStore.scene.isClipMode) {
      return;
    }
