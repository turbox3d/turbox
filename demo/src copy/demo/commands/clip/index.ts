...
import { Command, Action, ViewEntity, SceneEvent, EntityObject, Vec3, Vector3, SceneTool } from '@byted-tx3d/turbox';

import { ClipPointSymbol } from '../../common/consts/scene';
...
export class ClipCommand extends Command {
  ...
  protected onDragMove(viewEntity: ViewEntity, event: SceneEvent, tools: SceneTool) {
    ...
    const delta = new Vector3(sp.x, sp.y, sp.z)
      .sub(this.initPosition)
      .applyAxisAngle(new Vector3(0, 0, 1), (-this.target.parent!.rotation.z * Math.PI) / 180); // 全局delta施加旋转逆变换到模型本地坐标
    ...
    if (this.target.position.x < diagonalP.x) {
      ...
      if (position.x > diagonalP.x - MIN_D) {
        position.x = diagonalP.x - MIN_D;
      }
    }
    if (this.target.position.x > diagonalP.x) {
      if (position.x > size.x / 2) {
    ...
    if (this.target.position.y < diagonalP.y) {
      ...
      if (position.y > diagonalP.y - MIN_D) {
        position.y = diagonalP.y - MIN_D;
      }
    }
    if (this.target.position.y > diagonalP.y) {
      if (position.y > size.y / 2) {
    ...
    this.action.execute(
      () => {
        if (this.target) {
          ...
          let otherVerticalP = otherClipPoints.find(
            p => p.position.x === this.target!.position.x && p.position.y !== this.target!.position.y
          );
          let otherHorizonP = otherClipPoints.find(
            p => p.position.y === this.target!.position.y && p.position.x !== this.target!.position.x
          );
          if (!otherVerticalP) {
