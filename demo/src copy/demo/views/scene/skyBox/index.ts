...
import { Reactive, EntityObject, Mesh3D, MathUtils, g } from '@byted-tx3d/turbox';
...
@Reactive
export class SkyBoxViewEntity extends Mesh3D<ISkyBoxProps> {
...
export class SkyBox extends Mesh3D<ICubeProps> {
  ...
  updateVisible() {
    const { model } = this.props;
    (this.view.children as THREE.Mesh[])
      .filter(child => child.name !== 'bottom')
      .forEach(child => {
